...
// 使用可选认证（不强制要求登录，但如果有 token 就解析用户）
...
// ========== 处理好友申请（同意/拒绝） ==========
router.post('/request/:requestId/handle', async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ success: false, message: '请先登录' });
    }
...
router.get('/feed-posts', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: posts, hasNewPosts, lastFeedView });
  } catch (error) {
...
// ========== 标记 Feed 已查看 ==========
router.post('/feed-viewed', async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ success: false, message: '请先登录' });
    }
    
    await User.findByIdAndUpdate(req.user._id, { lastFeedViewAt: new Date() });
