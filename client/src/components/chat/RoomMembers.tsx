...
const RoomMembers = () => {
  ...
  const getRoleBadge = (role: string) => {
    if (role === 'owner') {
      return <span className="bg-yellow-100 text-yellow-700 text-xs px-2 py-0.5 rounded-full">群主</span>;
    }
    if (role === 'admin') {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-4 py-3 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold flex-1">成员列表</h1>
        </div>
      ...
      <div className="p-4 space-y-2">
        {members.map(member => (
          <div key={member._id} className="bg-white rounded-xl shadow p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-semibold">{member.personaId?.name}</span>
