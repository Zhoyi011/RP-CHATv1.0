      ...
      >
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          {results.map((p) => {
            ...
            return (
              <div key={p.id} className="p-4 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-3">
                  <AvatarFrame avatarUrl={p.avatar || ''} frameName={null} size="md" />
                  <div className="flex-1">
                    <div className="font-medium text-gray-900 dark:text-white">{displayName}</div>
                    <div className="text-xs text-gray-400 flex items-center gap-1">
                      <User className="w-3 h-3" /> {p.ownerUsername}
                    </div>
                  </div>
                  {p.isFriend ? (
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        maxLength={100}
                      />
                      <button onClick={() => handleSend(p.id, displayName)} className="px-2 py-1 bg-purple-500 text-white rounded-lg text-sm">
                        发送
                      </button>
                      <button onClick={() => setSelectedPersona(null)}><X className="w-4 h-4 text-gray-500" /></button>
                    </div>
                  ) : (
                    <button onClick={() => setSelectedPersona(p)} className="px-3 py-1.5 bg-purple-500 text-white rounded-lg text-sm">添加好友</button>
                  )}
                </div>
              </div>
            );
