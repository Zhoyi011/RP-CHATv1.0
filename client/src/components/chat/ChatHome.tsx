...
const MessageBubble: React.FC<{
...
}> = React.memo(({ message, isSelf, isMobile, navigate, selectedPersona, onTranslate, translatingMsgId, onLongPress, onReply }) => {
  ...
  const handleDragEnd = useCallback(() => {
    ...
    setSwipeOffset(0);
  }, [swipeOffset, onReply, message, getSenderDisplayName]);
  ...
  const getBubbleStyle = useCallback((): React.CSSProperties => {
    if (swipeOffset !== 0) {
      return {
...
const MessageList: React.FC<{
...
}> = React.memo(({ messages, user, selectedPersona, isMobile, navigate, onMessagesChange, onReply, onRecall, onDeleteSelf, onOpenRedPacket }) => {
  ...
  const handleTranslate = useCallback(async (content: string, msgId: string) => {
        ...
        return (
          <React.Fragment key={msg._id}>
            ...
            {(() => {
              ...
              // 🎨 表情消息
              if ((msg as any).isEmoji && (msg as any).emojiUrl) {
                ...
                return (
                  <motion.div
                    ...
                    animate="visible"
                    exit="exit"
                    className={`flex items-start gap-2 ${isSelf ? 'justify-end' : ''}`}
                  >
                    {!isSelf && (
                      <AvatarFrame
                        avatarUrl={msg.personaId?.avatar || ''}
                        frameName={getFrameNameFromUrl(msg.personaId?.avatarFrame || msg.personaId?.equipped?.avatarFrame)}
                        size="sm"
                        className="flex-shrink-0 cursor-pointer hover:scale-105 transition"
                        onClick={() => msg.personaId?._id && navigate(`/persona/${msg.personaId._id}`)}
                      />
                    )}
                    ...
                    <div className={`flex flex-col ${isSelf ? 'items-end' : 'items-start'}`}>
                      ...
                      <EmojiMessage
                        url={(msg as any).emojiUrl}
                        isGif={(msg as any).emojiUrl?.includes('.gif')}
                        size="md"
                      />
                    </div>
...
const ChatHome = () => {
  ...
  // 🔥 从全局获取数据（自动轮询）
  const { rooms, unreadTotal } = useAppData();
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [personasData, activePersonaRes] = await Promise.all([
        ...
        toast.custom((t) => (
          <div className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg p-3 flex items-center gap-3 cursor-pointer ${t.visible ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'}`}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{message.content || '[表情]'}</p>
            </div>
          </div>
    ...
    return (
      <div className="h-full flex flex-col bg-white dark:bg-gray-900">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!showUserList && !showAIChat && !loading && roomList.length === 0 && (
          ...
          {!showUserList && !showAIChat && roomList.length > 0 && (
            roomList.map(room => (
              <div 
                key={room._id} 
