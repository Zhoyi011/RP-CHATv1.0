    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-cyan-600 text-white sticky top-0 z-10 shadow-md">
        ...
        <div className="flex px-4 gap-1 overflow-x-auto">
          <button onClick={() => setActiveTab('basic')} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition ${activeTab === 'basic' ? 'bg-white text-blue-600' : 'text-white hover:bg-white/10'}`}>⚙️ 基本设置</button>
          <button onClick={() => setActiveTab('announcement')} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition ${activeTab === 'announcement' ? 'bg-white text-blue-600' : 'text-white hover:bg-white/10'}`}>📢 群公告</button>
          <button onClick={() => setActiveTab('members')} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition ${activeTab === 'members' ? 'bg-white text-blue-600' : 'text-white hover:bg-white/10'}`}>👥 成员管理</button>
          <button onClick={() => setActiveTab('titles')} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition ${activeTab === 'titles' ? 'bg-white text-blue-600' : 'text-white hover:bg-white/10'}`}>🏷️ 头衔管理</button>
        </div>
      </div>
      ...
      <div className={`${isMobile ? 'p-3' : 'p-4'} max-w-2xl mx-auto space-y-4`}>
        ...
        {activeTab === 'basic' && (
          <>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-5">
              ...
              <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" maxLength={30} />
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">群简介</label>
              <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none resize-none" rows={3} maxLength={200} placeholder="介绍一下这个群..." />
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-5">
              ...
              <div className="space-y-4">
                <label className="flex items-center justify-between cursor-pointer">
                  ...
                  <button onClick={() => setFormData({ ...formData, isPublic: !formData.isPublic })} className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${formData.isPublic ? 'bg-gradient-to-r from-blue-500 to-cyan-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
                ...
                <label className="flex items-center justify-between cursor-pointer">
                  ...
                  <button onClick={() => setFormData({ ...formData, requireApproval: !formData.requireApproval })} className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${formData.requireApproval ? 'bg-gradient-to-r from-blue-500 to-cyan-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
        ...
        {activeTab === 'announcement' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-5">
            ...
            <button onClick={handleSave} disabled={saving} className="mt-4 w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white py-2.5 rounded-xl font-medium hover:from-amber-600 hover:to-orange-600 transition disabled:opacity-50 shadow-md">
              {saving ? '保存中...' : '发布公告'}
            </button>
          </div>
        )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3">
                <button onClick={() => { setShowTitleModal(false); setSelectedMember(null); }} className="flex-1 px-4 py-2 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition">取消</button>
