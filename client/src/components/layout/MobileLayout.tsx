...
const MobileLayout: React.FC<Props> = ({ children }) => {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
      {/* 顶部导航栏 */}
      <div className="fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-b border-gray-100 z-20 shadow-sm safe-top">
        <div className="flex items-center justify-between px-4 py-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-1.5 text-gray-500 hover:text-blue-600 rounded-full hover:bg-gray-100 transition-all duration-200 active:scale-90"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* 底部 Tab 栏 */}
      <div 
        className={`fixed left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-gray-100 z-20 transition-all duration-300 safe-bottom ${
        ...
        <div className="flex justify-around items-center h-full">
          {tabs.map((tab) => (
            ...
            >
              ...
              <span className={`text-[11px] mt-0.5 ${activeTab === tab.name.toLowerCase() ? 'font-medium' : ''}`}>
                {tab.name}
              </span>
            </button>
          ))}
